<form method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <select class="form-control" required name="supplier">
            ...
            <option @if ($selected == 'Bliblu') selected @endif>Bliblu</option>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        @if ($method == 'edit' && !empty($item->foto))
            <br>
            <small>Foto saat ini: <img src="{{ asset('uploads/master_items/' . $item->foto) }}" width="100" height="100" style="object-fit: cover;"></small>
        @endif
    </div>
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="kategoris[]" multiple>
            @foreach($kategoris as $kategori)
                <option value="{{ $kategori->id }}" 
                    @if($method == 'edit' && $item->kategoris->contains('id', $kategori->id)) 
                        selected 
                    @endif>
        ...
        <small class="text-muted">Tahan Ctrl untuk memilih multiple kategori</small>
    </div>
